...
if(!empty($_POST)){
    ...
    if(isset($_POST['token'])){
        ...
        if($token==$SessionUser->token){
            $NewCompany = new CompanyObj();
            $NewCompany->companyname= $_POST['company'];
            $NewCompany->active= $_POST['active'];
            
            if($debug){
                echo 'Company ='.$NewCompany->company ." active=". $NewCompany->active;
            }
            
            $_ADOCompanies = new ADOCompanies();
            $_ADOCompanies->debug=$debug;
            
            $_ADOCompanies->AddNewCompany($NewCompany);
            if($debug){
                echo 'Record Added';
            }
